import pytest
from backend.tariff.config import CountryCodeRepoConfig
from backend.tariff.country_code_repo import CountryCodeRepo
...
@pytest.mark.asyncio
async def test_store_and_found_country_code() -> str:
    config = CountryCodeRepoConfig(postgres_table="test_country_code_table")
    system_config = SystemStoreConfig(postgres_table="test_system_table")
    repo = CountryCodeRepo(config, system_config)
    result = await repo.fetch_country_codes()
    results = await repo.find_country_codes("United States")
    assert results is not None
    assert isinstance(results, list)
    assert len(results) > 0
    assert "metadata" in results[0]
    metadata = results[0]["metadata"]
    assert metadata["countrycode"] == "840"
    assert metadata["iso3Code"] == "USA"
    # name should now be in content field, not in metadata
    assert results[0]["content"] == "United States"
    # Verify that name is not in metadata since it's the content column
...
async def test_drop_country_code_repo():
    ...
    system_store = SystemStore(system_config)
    await system_store.connect()
    await system_store.upsert_item("country_code_metadata_columns", meta_columns)
    
    # Verify the data exists
    metadata_from_system = await system_store.get_item("country_code_metadata_columns")
    ...
    system_store = SystemStore(system_config)
    await system_store.connect()
    metadata_after_drop = await system_store.get_item("country_code_metadata_columns")
